...
def get_parser():
    ...
    parser.add_argument('--save-iter', action='store_true', help='Save the model in every N interation (default is set by print freq)')
    ...
    while input is not None:
        ...
        if args.rank == 0 and i % args.print_freq == 0 and i > 1:
            ...
            save_checkpoint_iteration({
                'epoch': epoch + 1, 'iteration': i + 1, 'arch': args.arch, 
                'state_dict': model.state_dict(), 'optimizer' : optimizer.state_dict(),
                }, str(epoch + 1).zfill(3), str(i + 1).zfill(5))
...
def validate(val_loader, model, criterion, epoch, start_time):
    ...
    while input is not None:
        ...
        # compute output
        #with torch.no_grad():   # this requires pytorch >= 0.4.0
        output = model(input_var)
        loss = criterion(output, target_var)
    ...
    print(f'~~{epoch}\t{float(time_diff / 3600.0)}\t{top5.avg:.3f}\n')
...
def save_checkpoint_epoch(state, epoch_str, filename='checkpoint.pth.tar'):
    torch.save(state, filename)
    shutil.copyfile(filename, f'{args.save_dir}/checkpoint_{epoch_str}_done.pth.tar')

def save_checkpoint_iteration(state, epoch_str, iter_str):
    torch.save(state, f'{args.save_dir}/checkpoint_{epoch_str}_{iter_str}.pth.tar')
